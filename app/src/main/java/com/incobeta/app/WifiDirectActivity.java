...
public class WifiDirectActivity extends AppCompatActivity implements PeerListFragment.OnListFragmentInteractionListener
        ...
        if (!isWDConnected) {
            ...
            wifiP2pManager.discoverPeers(wifip2pChannel, new WifiP2pManager.ActionListener() {
                ...
                public void onSuccess() {
                    NotificationToast.showToast(WifiDirectActivity.this, "Peer discovery started");
                }

                @Override
                public void onFailure(int reasonCode) {
                    NotificationToast.showToast(WifiDirectActivity.this, "Peer discovery failure: "
                            + reasonCode);
                }
            });
        }
        else {
            Snackbar.make(v, "Already Connected", Snackbar.LENGTH_LONG)
                    .setAction("Action", null).show();
        }
    }
...
//        }
    ...
    protected void onResume() {
        ...
        LocalBroadcastManager.getInstance(WifiDirectActivity.this).registerReceiver(localDashReceiver,
                localFilter);
...
//        connListener.tearDown();
        ...
        Utility.clearPreferences(WifiDirectActivity.this);
        Utility.deletePersistentGroups(wifiP2pManager, wifip2pChannel);
        DBAdapter.getInstance(WifiDirectActivity.this).clearDatabase();
...
//                    appController.startConnectionListener(ConnectionUtils.getPort(LocalDashWiFiDirect.this));
                    appController.restartConnectionListenerWith(ConnectionUtils.getPort(WifiDirectActivity.this));

                    String senderIP = intent.getStringExtra(KEY_FIRST_DEVICE_IP);
                    int port = DBAdapter.getInstance(WifiDirectActivity.this).getDevice
                            (senderIP).getPort();
                    DataSender.sendCurrentDeviceData(WifiDirectActivity.this, senderIP, port, true);
                    isWDConnected = true;
                    break;
                case DataHandler.DEVICE_LIST_CHANGED:
                    ArrayList<DeviceDTO> devices = DBAdapter.getInstance(WifiDirectActivity.this)
                            .getDeviceList();
                ...
                case DataHandler.CHAT_REQUEST_RECEIVED:
                    DeviceDTO chatRequesterDevice = (DeviceDTO) intent.getSerializableExtra(DataHandler
                            .KEY_CHAT_REQUEST);
                    DialogUtils.getChatRequestDialog(WifiDirectActivity.this,
                ...
                case DataHandler.CHAT_RESPONSE_RECEIVED:
                    ...
                    if (!isChatRequestAccepted) {
                        NotificationToast.showToast(WifiDirectActivity.this, "Chat request " +
                                "rejected");
                    } else {
...
//                        DialogUtils.openChatActivity(LocalDashWiFiDirect.this, chatDevice);
                        NotificationToast.showToast(WifiDirectActivity.this, chatDevice
        ...
        switch (requestCode) {
            case DialogUtils.CODE_PICK_IMAGE:
                if (resultCode == RESULT_OK) {
                    ...
                    DataSender.sendFile(WifiDirectActivity.this, selectedDevice.getIp(),
...
//            appController.startConnectionListener(ConnectionUtils.getPort(LocalDashWiFiDirect.this));
            ...
            DataSender.sendCurrentDeviceDataWD(WifiDirectActivity.this, groupOwnerAddress, TransferConstants
    ...
    public void onListFragmentInteraction(DeviceDTO deviceDTO) {
        if (!isWDConnected) {
            ...
            wifiP2pManager.connect(wifip2pChannel, config, new WifiP2pManager.ActionListener() {
                ...
                @Override
                public void onFailure(int reasonCode) {
                    NotificationToast.showToast(WifiDirectActivity.this, "Connection failed. try" +
...
//            showServiceSelectionDialog();
            DialogUtils.getServiceSelectionDialog(WifiDirectActivity.this, deviceDTO).show();
        }
    }

    private void setToolBarTitle(int peerCount) {
        if (getSupportActionBar() != null) {
            String title = String.format("Wifi-Direct(%s)", String
                    .valueOf(peerCount));
